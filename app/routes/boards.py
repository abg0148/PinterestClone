...
def delete_board(board_id):
    ...
    if not board:
        flash('Board not found or you do not have permission to delete it.', 'danger')
        return redirect(url_for('dashboard.dashboard'))
    ...
    for pin in pins:
        if not pin.root_pin_id:  # that is the pin is not a repin
            root_post_ids.append(pin.post_id)
    
    if root_post_ids:
        posts.soft_delete_posts(root_post_ids)

    if request.method == 'POST':
        board.terminated_at = datetime.utcnow()  # Soft delete
        for pin in pins:
    ...
    # For GET request, render confirmation page
    return redirect(url_for('dashboard.dashboard'))